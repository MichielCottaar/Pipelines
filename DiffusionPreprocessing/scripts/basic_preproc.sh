...
entry_cnt=0
for entry in ${rawdir}/${basePos}_?.nii* ${rawdir}/${baseNeg}_?.nii*  #For each series, get the mean b0 and rescale to match the first series baseline
do
...
scount2=1
indcount=0
for entry in ${rawdir}/${basePos}_?.nii*  #For each Pos volume
do
...
indcount=0
scount2=1
for entry in ${rawdir}/${baseNeg}_?.nii* #For each Neg volume
do
	#Extract b0s and create index file
	basename=`imglob ${entry}`
...
${FSLDIR}/bin/fslmerge -t ${rawdir}/Pos `echo ${rawdir}/${basePos}_?.nii*`
${FSLDIR}/bin/fslmerge -t ${rawdir}/Neg `echo ${rawdir}/${baseNeg}_?.nii*`
